...
class Kutser:
    def __init__(self, im_aligned, bbox, oxy_band = 38,lower_oxy = 36, upper_oxy = 49, NIR_band = 47):
        """
        this method is applied for hyperspectral imagery
        ...
        :param oxy_band (int): band index for outside oxygen absorption band, which corresponds to 860.48nm
            see Kutser, Vahtmäe and Praks (2009)
        """
    ...
    def get_normalised_depth_D(self):
        """ D_norm"""
        D = self.get_depth_D()
        D_max = D.max()
        return D/D_max
    ...
    def get_glint_G(self):
        """
        to get the pure glint spectrum, the minimum D_norm is subtracted from the largest D_norm
        This method could be sensitive to outliers because only two pixels are being used to identify the glint spectrum
        returns values between 0 to 1
        """
        D_norm = self.get_normalised_depth_D()
        G = D_norm.max() - D_norm.min()
        return G
    ...
    def get_corrected_bands(self):
        ...
        for band_number in range(self.n_bands): #iterate across bands
            ...
            corrected_band = kutser(R,G,D_norm)
